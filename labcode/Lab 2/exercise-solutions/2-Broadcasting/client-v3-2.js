...
// ----------------------------------------------
function appendMessage(sender, text, timestamp) { // NEW ADDITION

  const msg = document.createElement('div');      // Create a new <div> for the message
  // ----- NEW FORMATTED MESSAGE -----
  msg.textContent = `[${timestamp}] ${sender}: ${text}`;  // NEW ADDITION
...
socket.on('message', (data) => {
  appendMessage(data.sender, data.text, data.timestamp);   // <-- NEW ADDITION
});
