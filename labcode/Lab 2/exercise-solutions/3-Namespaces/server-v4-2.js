...
// NEW ADDITION
// Helper function to compute total users
// across all namespaces
// This requires us to know in advance the 
// namespaces involved so we can hardcode 
// in in the array
const nameSpacesUsed = ['/general', '/sports', '/tech', '/movies'];
function getTotalUsers() {
  let totalUsers = 0;
  for (nsp of nameSpacesUsed) {
    totalUsers += io.of(nsp).sockets.size
  }
  return totalUsers;
}
...
function setupNamespace(namespaceName) {
  ...
  nsp.on('connection', (socket) => {
    ...
    // NEW ADDITION TO UPDATE TOTAL USERS
    nsp.emit('totalUsersUpdate', {total: getTotalUsers() });
...
setupNamespace('/tech'); // --> NEW ADDITION HERE
...
server.listen(PORT, () => {
  ...
  console.log('Namespaces available: /general, /sports, /movies, /tech');
});
