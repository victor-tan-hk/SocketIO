...
io.on('connection', (socket) => {
  ...
  socket.emit('message', { sender: 'Server', text: `Welcome ${clientName}!` });

  // Notify all *other* clients that a new client has joined
  socket.broadcast.emit('message', { sender: 'Server', text: `${clientName} joined the chat.` });
  ...
  socket.on('chatMessage', (msg) => {
    ...
    io.emit('message', { sender: clientName, text: msg });
  });
  ...
  socket.on('disconnect', () => {
    ...
    io.emit('message', { sender: 'Server', text: `${clientName} left the chat.` });
  });
});
